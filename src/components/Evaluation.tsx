...
export default function Evaluation({ assessments, inspections, library, profile, onSave, onDelete, onSaveLibrary, onDeleteLibrary, toast }: Props) {
  ...
  const { averageUnit, estimatedValue, minUnit, maxUnit, precision, medianUnit, standardDeviation, coefficientOfVariation, outlierIds } = summary;
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
        ...
        <div className="space-y-4">
          <section className="panel blueprint p-5 sm:p-6">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><div className="border-t border-line-soft pt-2"><span className="text-fog-500">Mediana</span><strong className="num block text-fog-200">{money(medianUnit)} / m²</strong></div><div className="border-t border-line-soft pt-2"><span className="text-fog-500">Desvio padrão</span><strong className="num block text-fog-200">{money(standardDeviation)} / m²</strong></div><div className="border-t border-line-soft pt-2"><span className="text-fog-500">Coef. variação</span><strong className="num block text-fog-200">{coefficientOfVariation.toFixed(1)}%</strong></div><div className="border-t border-line-soft pt-2"><span className="text-fog-500">Atípicos sugeridos</span><strong className="num block text-accent-300">{outlierIds.length}</strong></div></div>
            <p className="mt-4 border-t border-line-soft pt-3 text-[11px] leading-relaxed text-fog-600">Estimativa preliminar por média aritmética dos valores unitários homogeneizados. Os atípicos são apenas sinalizados pelo critério IQR; a inclusão ou exclusão deve ser decidida e justificada pelo profissional responsável.</p>
          </section>
