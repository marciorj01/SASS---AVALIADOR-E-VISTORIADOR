...
export default function FieldLog({ inspectionId, value, onChange }: { inspectionId: string; value?: InspectionFieldLog; onChange: (next: InspectionFieldLog) => void }) {
  const log = useMemo(() => value ?? emptyLog(inspectionId), [inspectionId, value]);
  const phase = PHASES.find((item) => item.id === log.phase) ?? PHASES[0];
  const update = (patch: Partial<InspectionFieldLog>) => onChange({ ...log, ...patch, updatedAt: todayISO() });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="panel p-5">
        <div className="mb-3 flex items-center justify-between gap-2"><div><p className="eyebrow">LEITURAS DE CONSUMO</p><h3 className="font-display text-xl font-semibold uppercase text-fog-100">Medidores</h3></div><Btn onClick={addReading}><IcPlus width={14} height={14} /> Leitura</Btn></div>
      ...
      <div className="panel p-5">
        <div className="mb-3 flex items-center justify-between gap-2"><div><p className="eyebrow">CONTROLE DE ACESSO</p><h3 className="font-display text-xl font-semibold uppercase text-fog-100">Chaves e controles</h3></div><Btn onClick={addKey}><IcPlus width={14} height={14} /> Chave</Btn></div>
      ...
      <div className="panel p-5"><p className="eyebrow">OBSERVAÇÕES DA FASE</p><TextArea value={log.notes} onChange={(e) => update({ notes: e.target.value })} placeholder="Registre condições gerais, pendências de entrega ou informações importantes da conferência..." className="mt-2 min-h-[100px]" /></div>
    </div>
  );
}
